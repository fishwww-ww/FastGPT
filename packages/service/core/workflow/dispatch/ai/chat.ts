...
import { getMsgSinJsonStr } from '../../../../../../../projects/app/src/service/support/outLink/wecom/utils';
...
async function streamResponse({
  ...
  wecomCrypto: ChatDispatchProps['wecomCrypto'];
  isFinished: boolean;
}) {
  ...
  if (!answer || answer.trim().length === 0) {
    ...
    return { answer: defaultAnswer, reasoning, finish_reason, usage };
  }

  if (isFinished) {
    const finishRes = plainAnswer(true, answer);
    const finishStamp = Math.floor(Date.now() / 1000).toString();
    const finishEncrypt = getMsgSinJsonStr(
      finishRes,
      finishStamp,
    ...
    const answerRes = plainAnswer(false, answer);
    const answerStamp = Math.floor(Date.now() / 1000).toString();
    const streamEncrypt = getMsgSinJsonStr(
      answerRes,
      answerStamp,
